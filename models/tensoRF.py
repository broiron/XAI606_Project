...
class TensorVMSplit(TensorBase):
    ...
    def get_optparam_groups(self, lr_init_spatialxyz_den = 0.02, lr_init_spatialxyz_app = 0.02, lr_init_network = 0.001):
        grad_vars = [{'params': self.density_line, 'lr': lr_init_spatialxyz_den}, {'params': self.density_plane, 'lr': lr_init_spatialxyz_den},
                     {'params': self.app_line, 'lr': lr_init_spatialxyz_app}, {'params': self.app_plane, 'lr': lr_init_spatialxyz_app},
                         {'params': self.basis_mat.parameters(), 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        return grad_vars
    ...
    def entropy_loss(self, alphas_map, acc_map,threshold):
        # alphas_map = alphas_map[:,:len(alphas_map[0])//2] # slicing half of the array for the memory cost.

        rays_prob = alphas_map / (torch.sum(alphas_map,-1).unsqueeze(-1)+1e-10)
        entropy_ray = -1*rays_prob*torch.log2(rays_prob+1e-10)
        entropy_ray_loss = torch.sum(entropy_ray,-1)
        mask  = (acc_map>threshold).detach()
        entropy_ray_loss *= mask
        entropy_ray_loss = torch.mean(entropy_ray_loss)
